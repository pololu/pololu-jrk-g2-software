...
static uint32_t jrk_mem_error_code_array[1] = { JRK_ERROR_MEMORY };

static char jrk_error_no_memory_msg[] =
  "Failed to allocate memory.";
jrk_error jrk_error_no_memory =
{
  .do_not_free = true,
  .message = jrk_error_no_memory_msg,
  .code_count = 1,
  .code_array = jrk_mem_error_code_array,
};

static char jrk_error_masked_by_no_memory_msg[] =
  "Failed to allocate memory for reporting an error.";
static jrk_error jrk_error_masked_by_no_memory =
{
  .do_not_free = true,
  .message = jrk_error_masked_by_no_memory_msg,
  .code_count = 1,
  .code_array = jrk_mem_error_code_array,
};

static jrk_error jrk_error_blank =
{
  .do_not_free = true,
  .message = NULL,
  .code_count = 0,
...
void jrk_error_free(jrk_error * error)
{
...
jrk_error * jrk_error_copy(const jrk_error * src_error)
{
  ...
  // Allocate memory.
  jrk_error * new_error = malloc(sizeof(jrk_error));
    ...
    (code_count != 0 && new_code_array == NULL))
  {
    ...
    free(new_message);
    free(new_code_array);
    return &jrk_error_masked_by_no_memory;
  }
...
// cannot be allocated.
static jrk_error * jrk_error_make_mutable(jrk_error * error)
{
  if (error == NULL) { error = &jrk_error_blank; }
  if (error->do_not_free)
  {
    error = jrk_error_copy(error);
  }
  return error;
}
...
// have been freed.
jrk_error * jrk_error_add_v(jrk_error * error, const char * format, va_list ap)
{
  if (format == NULL) { return error; }

  error = jrk_error_make_mutable(error);
...
jrk_error * jrk_error_add(jrk_error * error, const char * format, ...)
{
  va_list ap;
  va_start(ap, format);
  error = jrk_error_add_v(error, format, ap);
  va_end(ap);
  return error;
}
...
jrk_error * jrk_error_create(const char * format, ...)
{
  va_list ap;
  va_start(ap, format);
  jrk_error * error = jrk_error_add_v(NULL, format, ap);
  va_end(ap);
  return error;
}

bool jrk_error_has_code(const jrk_error * error, uint32_t code)
{
...
const char * jrk_error_get_message(const jrk_error * error)
{
  if (error == NULL)
  {
    return "No error.";
...
// Convert a libusbp_error into a jrk_error and free the libusbp_error.
jrk_error * jrk_usb_error(libusbp_error * usb_error)
{
  if (usb_error == NULL) { return NULL; }

  // Create the error and copy the message from libusbp.
  jrk_error * error = jrk_error_create("%s", libusbp_error_get_message(usb_error));

  if (libusbp_error_has_code(usb_error, LIBUSBP_ERROR_MEMORY))
  {
    jrk_error_add_code(error, JRK_ERROR_MEMORY);
  }

  if (libusbp_error_has_code(usb_error, LIBUSBP_ERROR_ACCESS_DENIED))
  {
    jrk_error_add_code(error, JRK_ERROR_ACCESS_DENIED);
  }

  if (libusbp_error_has_code(usb_error, LIBUSBP_ERROR_TIMEOUT))
  {
    jrk_error_add_code(error, JRK_ERROR_TIMEOUT);
  }

  if (libusbp_error_has_code(usb_error, LIBUSBP_ERROR_DEVICE_DISCONNECTED))
  {
    jrk_error_add_code(error, JRK_ERROR_DEVICE_DISCONNECTED);
  }
