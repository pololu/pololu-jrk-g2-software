...
void jrk_string_free(char * data)
{
  free(data);
}

void jrk_string_setup(jrk_string * str)
{
  assert(str != NULL);
  str->data = malloc(1);
  if (str->data != NULL)
...
void jrk_string_setup_dummy(jrk_string * str)
{
  assert(str != NULL);
  str->data = NULL;
...
void jrk_sprintf(jrk_string * str, const char * format, ...)
{
  assert(format != NULL);
...
uint8_t jrk_string_to_i64(const char * str, int64_t * out)
{
  *out = 0;
