...
def generate_settings_accessor_prototypes(stream)
  Settings.each do |setting_info|
    ...
    stream.puts "TIC_API"
    stream.puts "#{type} jrk_settings_get_#{name}(jrk_settings *);\n"
    stream.puts
  end
end
...
def generate_settings_cpp_accessors(stream)
  Settings.each do |setting_info|
    name = setting_info.fetch(:name)
    type = setting_integer_type(setting_info)
    stream.puts "void jrk_settings_set_#{name}(#{type} #{name})"
    stream.puts "{"
...
def generate_buffer_to_settings_code(stream)
  Settings.each do |setting_info|
    ...
    stream.puts "  jrk_setting_set_#{name}(settings, #{name});"
    stream.puts "}"
    stream.puts
  end
end
...
def generate_settings_to_buffer_code(stream)
  Settings.each do |setting_info|
    ...
    type = setting_integer_type(setting_info)
    stream.puts "{"
    stream.puts "  #{type} #{name} = jrk_settings_get_#{name}(settings, #{name});"
...
def generate_settings_file_parsing_code(stream)
  Settings.each do |setting_info|
    ...
    if type == :enum
      ...
      stream.puts "  if (!jrk_name_to_code(jrk_#{name}_names, value, &#{name}))"
      stream.puts "  {"
      ...
      stream.puts "  if (tic_string_to_i64(value, &#{name}))"
      stream.puts "  {"
...
def generate_settings_file_printing_code(stream)
  Settings.each do |setting_info|
    ...
    pf = setting_printf_format(setting_info)
    stream.puts "{"
    stream.puts "  #{int_type} #{name} = tic_settings_get_#{name}(settings);"
    if type == :enum
      stream.puts "  const char * str = \"\";"
      stream.puts "  jrk_code_to_name(jrk_#{name}_names, #{name}, &str);"
      stream.puts "  tic_sprintf(&str, \"#{name}: %s\", str);";
    else
      stream.puts "  tic_sprintf(&str, \"#{name}: %#{pf}\", #{name});"
    end
    stream.puts "}"
    stream.puts
  end
